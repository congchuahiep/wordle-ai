...
class PlayFirstStrikeTest(unittest.TestCase):
    ...
    def run_test_on_dataset(self, dataset_name) -> tuple[float, float]:
        ...
        for target in words:
            ...
            with redirect_stdout(f):  # Chặn mọi print bên trong play_first_strike
                result = play_first_strike(
                    self.word_list, target_word=target, max_attempts=6
                )
    ...
    def test_random(self):
        ...
        logger.info(f"<<<Random>>>> avg tries: {avg_tries}")
        logger.info(f"<<<Random>>>> success rate: {success_rate} %")
        print(f"<<<Random>>>> Số lượt đoán trung bình: {avg_tries}")
        print(f"<<<Random>>>> success rate: {success_rate} %")
        print()
